...
export function RigDialogCompartmentsTab({
					...
					return (
						<>
							<AccordionItem value={group.key}>
								...
								<AccordionContent>
									<FieldGroup>
										{compartments[group.key] &&
											compartments[group.key].map((compartment) => {
												return (
													<Field
														key={compartment.reactKey}
														...
														{mode === "view" ? (
															<div className="flex items-center gap-3">
																...
																<p className="text-sm">{compartment.name}</p>
															</div>
														) : (
															<>
																<FieldLabel
																	className="	"
																	htmlFor={compartment.reactKey}>
																	{compartment.position}{" "}
																</FieldLabel>
																<Input
										...
										<FieldGroup>
											<Field orientation="horizontal">
												<Button
													type="button"
													variant="outline"
													onClick={() => handleAddNewCompartment(group.key)}>
													<PlusIcon className="h-4 w-4" /> Add Compartment
												</Button>
											</Field>
										</FieldGroup>
									)}
	...
	function handleRemoveCompartment(
		reactKey: string,
		groupKey: CompartmentGroupKey,
	) {
