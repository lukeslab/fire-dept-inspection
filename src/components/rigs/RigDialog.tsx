...
export type CompartmentsState = Record<
	CompartmentGroupKey,
	CompartmentFormInput[]
>
...
export function RigDialog({
	...
	async function handleRigDialogSubmit(
		...
		const flatCompartments = Object.entries(compartments).flatMap(
			([groupKey, groupCompartments]) =>
		...
		if (payload.compartments.length === 0) {
			setValidationErrors((prev) => [
				...prev,
				"Please enter at least one compartment.",
			])
			setIsSubmitting(false)
			return
		}
